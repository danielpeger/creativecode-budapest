...
export default function EventPagePreview({ entry, widgetsFor }) {
  return (
    <StylesheetInjector>
      <Root>
        <GlobalStyle />
            ...
            <InfoList>
              ...
              {entry.getIn(['data', 'location']) &&
                <li>
                  ...
                  <a href={entry.getIn(['data', 'googleMapsLink']) ? entry.getIn(['data', 'googleMapsLink']) : ''}>{entry.getIn(['data', 'location'])}</a>
                </li>
              }
              {entry.getIn(['data', 'meetupEventLink']) &&
                <li>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
              ...
              {entry.getIn(['data', 'facebookEventLink']) &&
                <li>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
          ...
          <SpeakerList>
            {entry.getIn(['data', 'speakers']) &&
              entry.getIn(['data', 'speakers']).map((speaker, index) => (
                <SpeakerElement key={index}>
                  {speaker.getIn(['data', 'image']) && speaker.getIn(['data', 'name']) &&
                    <Image
                      src={speaker.getIn(['data', 'image'])}
                      width={260}
                      aspectRatio={1}
                      alt={speaker.getIn(['data', 'name'])}
                      customTransformations="g_face,c_thumb,z_0.7"
                    />
                  }
                  <div>
                    {speaker.getIn(['data', 'name']) &&
                      <NameElement>{speaker.getIn(['data', 'name'])}</NameElement>
                    }
                    {speaker.getIn(['data', 'bio']) &&
                      <div
                        dangerouslySetInnerHTML={{ __html: MarkdownToHtml(speaker.getIn(['data', 'bio'])) }}
                      ></div>
                    }
                  </div>
                </SpeakerElement>
